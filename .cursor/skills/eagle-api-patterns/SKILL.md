...
通过浏览器导航 Eagle 自定义协议 URL（`window.location.href`），无需额外 API 调用：

| URL | 用途 | 模块 |
|-----|------|------|
| `http://localhost:41595/item?id={itemId}` | 定位到具体作品 | `eagle/deep-link.js` |
| `http://localhost:41595/folder?id={folderId}` | 定位到文件夹 | `eagle/deep-link.js` |

统一入口：`openInEagle({ itemId, folderId })` — itemId 优先，folderId 作 fallback。
