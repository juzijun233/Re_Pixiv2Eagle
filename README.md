...
本仓库使用 **esbuild** 作为构建管线：源码位于 `src/`，构建产物为单文件用户脚本 `dist/Pixiv.js`，可直接导入 Tampermonkey。
...
npm run build  # 生产构建，输出 dist/Pixiv.js
```

## 安装

1. 运行 `npm run build` 生成 `dist/Pixiv.js`。
2. 打开 Tampermonkey → 新建脚本 → 粘贴 `dist/Pixiv.js` 全部内容，或直接以本地文件方式导入。
